// =============================================================================
// Concur Invoice KG — WORKFLOWS group, RUN A build workflow, 2026-08-31.
// Pages: Workflows (one page, seven tabs) + Feature Hierarchies.
//
// Adapted from workflows/2026-08-31_kg-group-3.mjs — the exact script as run for Group 3
// (20 agents, 0 errors, 49 fields, validator exit 0). Only the per-group knobs were turned,
// plus the Workflows findings carried in from the page recon and BOTH ITS CRITICS.
...
//
// WHY THIS GROUP IS FOUR PAGES AND NOT THIRTEEN
// INVOICE-CONFIG-MAP.md recorded "13 pages of workflow config" UNDER ONE LEFT-MENU ENTRY, which
// is the tell: it was counting SURFACES. A dedicated recon
// (workflows/2026-08-31_kg-workflows-page-recon.mjs, run wf_f4d39441-6cb: six blind sweeps, a
// merge, two adversarial critics; 9 agents, 0 errors, 1.97M tokens) re-derived the roster.
// ALL SIX SWEEPS converged: Workflows is ONE admin page with SEVEN TABS — the Audit Rules
// precedent, not Forms and Fields. The pagehood critic ENDORSED the roster outright ("no page
// added, removed, merged or split") after reproducing every mechanical claim to the byte.
// The 13 reconcile as 7 tabs + 3 wizard pages (General/Steps/Step Rules) + 3 settings tables.
// Every one is corpus-attested; not one is a page.
// THREE adjacent left-menu pages the lost map never counted at all: Email Reminders,
// Delegate Configurations, Feature Hierarchies.
// Full report: output/reports/2026-08-31_workflows-recon/
//
// RUN A IS THIS SCRIPT: Workflows + Feature Hierarchies (94 unique files, 234,519 B, ~105 est.
// fields). They are paired because they SHARE SOURCE FILES — step-1-define-the-cost-object-
// approver-hierarchy-6e7b7dc6.md and understand-the-hierarchy-b65d7089.md (a 69-row catalog two
// runs would otherwise read twice) — and because Feature Hierarchies carries the Workflows
// build's hard forward reference ("You must define the authorized approver hierarchy").
// RUN B IS SEPARATE: Email Reminders + Delegate Configurations, merged WITH --patch.
// The split can never cut through Workflows itself: --patch replaces a page's fields wholesale.
//
// THE ONE THING THE TWO RECON CRITICS DISAGREED ABOUT, AND HOW IT WAS SETTLED.
// The completeness critic said ADD a fifth page, `authorized-approval-limits`. The pagehood
// critic endorsed four and added none. Per the method, a split is TWO FINDINGS, not a tie.
// Adjudicated against the corpus and the graph:
//   * Completeness is RIGHT ON THE FACT. The roster deferred that surface claiming the corpus
//     defers it to an external Shared guide. That claim is FALSE: user-administrator-fcfd570c.md
//     (4,603 B) and user-administration-8b167b96.md (2,314 B) document it directly and all three
//     of the critic's anchor quotes verify with grep -F. The per-user approval LIMIT — the value
//     deciding whether limit-approval can happen at all — has no home anywhere in the graph.
//   * Pagehood is RIGHT ON THE ACTION. The click path is "Administration > Company > Company
//     Admin > User Administration" — OUTSIDE the Invoice admin menu. Verified precedent: ALL 20
//     built pages sit under Administration > Invoice, zero exceptions; and five prior groups met
//     User Administration / User Permissions / Administration > Company > User Administration
//     EIGHT times and left every one as an unresolved endpoint rather than building it.
// VERDICT: no fifth page in this run. Building it would be a SCOPE EXPANSION of the whole graph
// — Luke's call, not a page-roster conclusion — and making it silently inside a group build is
// exactly the re-homing error the Group 3 completeness critic forbade. The deferral's REASON is
// corrected from the false external-deferral claim to the true scoping argument, and the click
// path, the seven named fields and the valid-value semantics (0 = in chain but no final
// authority; null = unlimited) are recorded as scoped debt so it is DEFERRED, NOT DELETED.
//
...
//                  Run A is a NEW label, so it is ABSENT here and the merge runs WITHOUT
//                  --patch. RUN B MUST SET IT.
//   step id prefix - the Synthesize/steps prompt hard-codes the group tag
//   PARTS        - the current session's scratchpad path
//
// THE GROUP LABEL IS "Workflows" WITH NO "Group N", AND THAT IS DELIBERATE. merge-group.py
// derives gtag from /Group (\d+)/, so ANY label containing "Group 2" (the lost map files
// Workflows under Group 2) would derive gtag '2' and mint dep.g2.001... COLLIDING with the
// built Group 2 edges — and a non-patch merge under that label would DELETE Audit Rules'
// 91 fields. "Workflows" derives gtag 'workflows' and step prefix 'grpworkflows-'. Verified.
//
// THREE TOOLCHAIN DEFECTS WERE FIXED BEFORE THIS RUN (grep the whole toolchain, not just the
// workflow file — a documented knob list goes stale silently):
//   1. bin/assemble-parts.py hard-coded 'grp5b-unnamed' as the step-id fallback.
//   2. bin/merge-group.py ALL_GROUPS had NO Workflows entry, so groupsRemaining omitted it and
//      merging Group 6 would have flipped meta.status to COMPLETE with Workflows unbuilt.
//   3. bin/merge-group.py carried only 5 page keys through the merge — `tabs` was NOT among
//      them, so this build's seven tabs would have been SILENTLY DROPPED. page.forms-and-fields
//      has tabs only via a hard-coded special case in apply-corrections.py:322. Generalised.
//
// NOTE ON url: both pages carry url: ''. The corpus publishes NO .asp URLs, and the original
// map preserved the Workflows row with its path field literally reading "(path not preserved in
// handoff)". The click path is the only navigation knowledge that will exist for these pages,
// which is why navPathEvidence matters more here than anywhere else in the build.
//
// NBSP NAVIGATION HAZARD — GROUP-LEVEL, NOT A FOOTNOTE (recon pagehood critic, finding 6).
// 57 files corpus-wide render menu paths with the '>' glyph ENTIRELY ABSENT, separators being
// U+00A0 U+00A0 ("AdministrationM-BM- M-BM- Invoice" under cat -A); 84 files use the arrow.
// ~40% of nav sentences lose the arrow. Load-bearing paths exist ONLY in the NBSP form,
// including the sentence anchoring inherited finding (1). NEVER grep -F across a menu arrow;
// any nav census must run BOTH separator forms or declare itself incomplete.
//
// See docs/2026-08-31_HANDOFF-KG-BUILD-v2.md and docs/SCHEMA.md.
// =============================================================================

export const meta = {
  name: 'kg-workflows-run-a',
  description: 'Concur Invoice KG - Workflows Run A: Workflows (one page, seven tabs), Feature Hierarchies',
  phases: [
...
const PARTS = '/tmp/claude-1000/-mnt-c-Users-manci/bc53169c-7f0a-473a-a07f-cf6d37ca509c/scratchpad/wf-a-parts'
...
const PAGES = [
  {
    ...
    name: 'Workflows',
    url: '',
    seeds: 'THE BIGGEST SINGLE PAGE IN THE PROJECT SO FAR - 86 files, ~222,500 raw bytes, est. ~100 fields, which would put it above the built Audit Rules page (91). Budget accordingly and DO NOT SAMPLE. ### IDENTITY, SETTLED BY THE RECON AND ENDORSED OUTRIGHT BY ITS PAGE-HOOD CRITIC - DO NOT RE-LITIGATE: ONE PAGE, SEVEN TABS. This is the AUDIT RULES precedent (one page, several tabs), NOT the Forms and Fields precedent. Proof, three independent ways: (i) LEFT-MENU CENSUS over both guide dirs (1,859 files) returns 30 distinct labels; "Workflows" scores 8, and Approval Statuses / Authorized Approvers / Email Notifications / Workflow Settings / Confirmation Agreements / Reason Category and Codes / Purchase Request Settings / Purchase Order Settings score ZERO between them. "Steps tab" returns ZERO corpus-wide. (ii) ONE ROLE GATE for the whole surface, stated TWICE verbatim as "permissions for the Workflows section in Administration" - never per tab. (iii) A July 2025 release note organises its content under four headings - "Forms and Fields Page", "Group Configurations Page", "Policies Page", "Workflows Page" - the first three already pages in this graph. ### THE SEVEN TABS, each with a verified click path from inside Workflows: Workflows, Settings, Email Notifications, Approval Statuses, Authorized Approvers, Confirmation Agreements, Reason Category and Codes. FOUR SUB-TABS: Approval Statuses > {Invoice, Purchase Request}; Authorized Approvers > {Configuration, Authorized Approver List}. A THREE-PAGE MODAL WIZARD over the workflow OBJECT: General -> Steps -> Step Rules, which returns you to the tab ("When done with all rules and actions, click Done on the Workflows tab.") - a modal sequence, NOT navigation. DO NOT CREATE PAGE NODES FOR ANY TAB, SUB-TAB, WIZARD PAGE OR DIALOG. ### EMIT THE TABS AS PAGE-LEVEL DATA. Set tabs to the seven names with tabsSourceQuote/tabsSourceFile, and add one navPathAlternates entry per tab and sub-tab in the existing graph idiom "Workflows > Approval Statuses (tab)". THE MERGE TOOL WAS FIXED ON 2026-08-31 TO CARRY tabs THROUGH - before that it would have dropped them silently. Extend aliases with: Workflow Settings, Settings tab, Purchase Request Settings, Purchase Order Settings, Approval Statuses, Authorized Approvers, Email Notifications, Confirmation Agreements, Reason Category and Codes - so future endpoints against those names resolve. ### PAGE vs OBJECT. The page LISTS and EDITS workflow objects; a workflow is an OBJECT and there can be many. The two create-a-new-workflow files were diffed by loio and are genuinely two topics, NOT a gerund/imperative republish: 15992497 (2,017 B) is the conceptual note and carries the only nested nav phrasing in the corpus ("the General page in the Workflows tab of the Workflows tool"); 554e86aa (14,383 B) is the full General-page field procedure and is the DENSEST FILE IN THE GROUP. ### THE FOUR workflow-guides-*.md FILES WERE DIFFED: four distinct loio, four distinct body md5, IDENTICAL 4-row payload listing four EXTERNAL setup guides. They are why this area LOOKS like four pages - SAP split the DOCUMENTATION by topic, not the UI. No node, no fields. ### INHERITED FINDING (1), CONFIRMED AS ANSWER (a) AND STRENGTHENED BY THE CRITIC: ALL THREE settings-family members live on WORKFLOWS > SETTINGS TAB - invoice-settings-cace748d.md, purchase-request-settings-b0bce285.md, purchase-order-settings-a5a997b4.md. Family cohesion is mechanically provable: grep -rl "apply globally to" over BOTH guide dirs returns EXACTLY THOSE THREE FILES AND NOTHING ELSE. Positive placement four ways, the strongest being the Workflows guide OWN revision history (general-information-8b3b0308.md) logging "Addition of two new options to the Settings tab:" followed by two rows that are literally rows 4 and 5 of the packed raw <table> inside invoice-settings-cace748d.md. THIS IS A LABEL COLLISION WITH THE BUILT INVOICE SETTINGS PAGE, NOT REBUILD DEBT: zero of the 486 graph fields cite any of the three files, and the built page 13 fields come from available-invoice-settings-8b3411f0.md, a DIFFERENT file. NAMING RULE, LOAD-BEARING: name these controls under "Settings tab" / "Workflow Settings" and NEVER "Invoice Settings", or the graph merges two different surfaces sharing a title. NOTHING MOVES OFF THE BUILT INVOICE SETTINGS PAGE. ### BUILD FLAG - PACKED RAW TABLE, YOU WILL LOSE THREE SETTINGS IF YOU MISS IT: invoice-settings-cace748d.md returns grep -c "<tr" = 1 but grep -o "<tr" | wc -l = 3 and "<p" = 6. It is TWO markdown rows PLUS THREE packed into ONE raw <table> on a single line: "Prevent this payment request submission when exception level exceeds X", "Filter payment request items to those that are applicable to Cost Object", "Allow processor to recall a payment request to last processor step". FIVE settings, not three. Settings-tab payload = 5 + 3 (PR) + 1 (PO) = NINE. A markdown-only extractor mis-reads the very file that settles finding (1). ### BUILD FLAG - PROSE RANGE NO DIGIT REGEX WILL FIND: "Type a number from one to 99." appears once in EACH of the three settings files. The built Exceptions page spells the same range "0 and 99" - record that as a genuine cross-page discrepancy, do not reconcile it. ### BUILD FLAG - LABEL DRIFT ON TWO SETTINGS-TAB CONTROLS (recon pagehood critic, finding 9). general-information-8b3b0308.md names them "Filter invoice items to those that are applicable to Cost Object" and "Allow processor to recall an invoice to last processor step"; invoice-settings-cace748d.md names the SAME two controls "Filter payment request items..." and "Allow processor to recall a payment request...". Same control, two labels, invoice <-> payment request. A Chromium driver locates checkboxes by visible label, so RECORD BOTH FORMS as aliases or the automation fails against a tenant rendering the other. ### CRITIC CORRECTION TO THE SEED LIST - THE EMAIL NOTIFICATIONS CATALOG WAS MIS-SEEDED AND ITS REAL CATALOG WAS MISSING: email-notifications-8b3dbad4.md IS THE GUIDE REVISION HISTORY, NOT A FIELD ROSTER (it contains exactly ONE variable). The actual catalog is variables-for-invoices-or-purchase-requests-26e917cb.md - a Field|Variable|Description table with 18 DISTINCT VARIABLES in nine label/value pairs (%L_EmpName%/%EmpName%, %L_PayStatus%/%PayStatus%, %L_RequestDate%, %L_RequestName%, %L_StepApprovalStatus%, %L_SubmitDate%, %L_SubmittingUserName%, %L_Total%, %L_WhoChanged%) - and its semantics parent general-information-00eca1ce.md carries the rule that makes the set usable: adding an "L" and underscore to the front of a variable name substitutes the field LABEL. BOTH ARE SEEDED BELOW. Enumerate all 18. ### CRITIC CORRECTION - FOUR REVISION-HISTORY FILES ARE IN THE SEED SET AND 37% OF THIS PAGE MEASURED TABLE PAYLOAD IS REVISION HISTORY. general-information-8b3b0308.md (139 rows), cost-object-approval-8b3d1e0f.md (85), authorized-approvers-8b3c26cc.md (64), email-notifications-8b3dbad4.md (61) all OPEN WITH "Revision History" and occupy positions #1/#3/#5/#6 by row count in the whole set. THE SINGLE LARGEST "CATALOG" IN THIS GROUP IS A REVISION HISTORY. MINE THEM FOR STRUCTURE, NAMES, DATES AND HISTORY CLAIMS ONLY - NEVER AS A FIELD ROSTER. general-information-8b3b0308.md MUST STAY: it is the proof for finding (1). Real payload is ~605 table units, not 954. ALSO: the real Authorized Approvers content is in the duplicate-title twin authorized-approvers-8b3ab7ad.md, seeded below - prefer it. ### CRITIC CORRECTION - FOUR TIMEOUT FIELDS THE RECON WRONGLY CALLED HOMELESS. The roster deferred about-vendor-approval-workflow-step-and-timeout-b3d1bd2c.md as having no click path. The family DOES have a fully nested one: "the Approval Time Expired Action list on the General page in the Workflows tab of the Workflows tool". create-a-new-workflow-554e86aa.md documents FOUR General-page timeout fields - Approval Time Expired Action, Expire After This Many Days, Assignment Timeout Action, Allow Timeout Extensions for This Many Days - plus the conditional "This option is only available if either the Approval Time Expired Action or Assignment Timeout Action options are enabled." EXTRACT ALL FOUR. Record b3d1bd2c "Workflow Step Timeout Duration"/"Approval Action" as a LABEL CONTRADICTION against them, not as orphan fields. ### CRITIC CORRECTION - administrator-experience-8b302852.md was cited by the roster but seeded nowhere. It carries a third independent page enumeration, the field "Require Hardcopy Receipts", and a CONDITIONAL-VISIBILITY DEPENDENCY: enabling Require Hardcopy Receipts on the Workflows page reveals the "Hold for Invoice Hard Copy" step on BOTH the Steps page and the Step Rules page. "Hardcopy" occurs exactly ONCE in the entire 1.84 MB graph today. Seeded below; emit that edge. ### AUDIT RULES DEDUPE - DO IT ON SOURCE FILES, NOT ON THE FIELD LIST. The condition editor is explicitly shared ("The Condition page in the Workflows tool is similar to the Condition Editor page used in the Audit Rules tool"). BUT the built Audit Rules 91-field roster is itself internally duplicated ~8 times (seven labels appear twice or three times, e.g. "B: Data Object" as condition_data_object_left + data_object + condition_data_object_b), so deduping against it PROPAGATES that error. Dedupe on FILES: the-condition-page-5d4ea870.md is the AUDIT RULES side (already mined, 16 fields, correctly NOT seeded here); conditional-expressions-and-the-condition-page-4d98af34.md is the WORKFLOWS side, seeded here, cited by zero graph fields. ### TOOLS-GUIDES IS NOT WRITTEN OFF. grep for "Workflows tool/page/tab" over tools-guides returns 0, but SIXTEEN tools-guides files say "Invoice Configuration administrator". admin-guides is the FIELD SOURCE; tools-guides is checked for CORROBORATION via "Invoice Configuration administrator" / "(left menu)" / "Invoice Processing Admin". how-single-step-approval-workflow-works-40145f24.md is seeded - it is the only topic tying the Authorized Approvers link, the approval-limit value and the skip-step rule into one narrative. DO NOT SEED either the-query-builder-and-the-condition-editor-*.md file (af058a80 / e10473f9, duplicate titles, different loio, ~19 KB of table): they are the Processor RUNTIME Query Builder, not config - name them in notes as do-not-confuse. ### uiVariant IS UNDETERMINED AND THAT IS THE HONEST ANSWER - INHERITED FINDING (6), ADDED BY THE RECON COMPLETENESS CRITIC. There is NO *-new-experience-* twin for any workflow file. But the guide own revision history, dated September 19 2025, says "Updated images and text to the new UI for the Workflows Tool" - so the missing twin is a REPLACEMENT (revised in place), not a coverage gap. Against that, tools-guides/how-single-step-approval-workflow-works-40145f24.md says the admin "uses the Authorized Approvers link from workflows in the classic interface". TWO DATED STATEMENTS, OPPOSITE DIRECTIONS, ONE CORPUS VERSION. Set uiVariant "undifferentiated", EMIT THIS AS A CONTRADICTION NODE, and say plainly in verifyNotes that the corpus cannot settle which UI generation the click paths describe. This matters more than any single field: click paths are the ONLY navigation knowledge this group will ever have, so if they describe a superseded UI every navPath is wrong at once. DO NOT CLAIM uiVariant "both" - three unearned "both" claims are already outstanding debt. TEXT TRAP: that revision line misspells "Worflows" (once corpus-wide). ### NINE UNRESOLVED GRAPH ENDPOINTS ALREADY NAME THIS PAGE and three of them PRE-COMMIT FIELD NAMES you must reconcile against, not reinvent: dep.g1.021 "workflow deletion", dep.g1.026 "Workflow", dep.g2.014 "Settings", dep.g4.045 "invoice submission for approval" (an e-Bunsho timestamp precondition), dep.g2ar.021 + dep.g2ar.023 "workflow_first_step", dep.g2ar.022 "workflow_approval_steps", dep.g2ar.024 + dep.g2ar.026 "exception_level_limit". Also dep.g2.012 -> {Purchase Request Settings, "Prevent purchase request submission when exception level exceeds X"} and dep.g2.013 -> {Purchase Order Settings, "Prevent purchase order transmission when exception level exceeds X"} resolve to the Settings tab. ### CONTRADICTIONS THE RECON RECORDED - CARRY THEM, DO NOT RECONCILE: C1 Settings called "Settings tab" vs "Settings section of the Workflows tool" vs "Settings page" vs "Workflow Settings page". C2 Authorized Approvers under FIVE labels. C3 "Workflows tab" (14 topics) vs "Workflow tab" (1) - and page-level singular/plural too ("On the Workflow page" and "on the Workflows page" in adjacent sentences of administrator-experience-8b302852.md). C4 Email Notifications plural vs singular. C5 Confirmation Agreements plural vs singular. C6 wizard vocabulary (General page / General step / Edit Workflow page; Add Workflow Step page vs window). C7 DEPTH COLLAPSE - two topics put General-page fields "on the Workflows page"; do NOT create a second node. C8 NAV DEPTH - 5 topics use the Invoice Processing Admin middle node, 8 omit it; record BOTH, automation tries the middle node first. C9 THE ROLE-GATE CONTRADICTION, UNRESOLVED AND THE MOST IMPORTANT FOR AUTOMATION: creating-a-reason-code-b2b61596.md says the Invoice Configuration administrator (Restricted) role IS REQUIRED to use the options on the Reason Category and Codes tab and walks through New -> Save, while the July 2025 release note says that same role is READ-ONLY on Workflows with New/Copy/Remove disabled. If the driver ever hits a permission wall on Workflows, that tab is the likely cause. ### THREE EXISTING-GRAPH DEFECTS TO REPORT, NOT FIX HERE: dep.g2.011 targets page "Invoice Settings" while citing invoice-settings-cace748d.md and should target Workflows > Settings tab; dep.g2ar.025 targets "Invoice Settings" on a quote naming Workflows and Settings and duplicates dep.g2ar.024; and page.exceptions.navPathAlternates contains "Administration > Invoice > Workflows > Settings", which is not a route to Exceptions.',
  },
  {
    id: 'feature-hierarchies',
    name: 'Feature Hierarchies',
    url: '',
    seeds: 'SMALL, SHARED, AND THIN ON PURPOSE - 12-file core, 23,239 B, EXPECT ROUGHLY FIVE FIELDS (Source List, Level, Segment Name, the feature-name selector, Modify Hierarchy). THE BUILD IS BEING TOLD THIS BEFORE IT STARTS SO IT DOES NOT MANUFACTURE FIELDS AROUND THE GAP - that is the Budget Configuration failure mode. Record coverage "partial" and say WHY in verifyNotes: SAP files the substance in the external "Shared: Feature Hierarchies Setup Guide", which is NOT in this corpus. Thin with a documented reason is a correct, publishable answer. ### IT IS A REAL PAGE AND IT BELONGS TO NEITHER WORKFLOWS NOR ROUTING CONFIGURATION - settles inherited finding (3), which the Group 3 recon left unclaimed. Found by ALL SIX SWEEPS, all reaching the same verdict. Own left-menu entry: "Click Feature Hierarchies (left menu)." - one of only TWO unbuilt labels in the corpus-wide 30-label census. Own role gate, worded exactly like the Workflows gate and stated IN CONTRAST to it inside a single three-step flow (step 1 = Feature Hierarchies permission, step 3 = Workflows permission): two gates, two peer surfaces, in SAP own contrast. It is never a tab of anything. ### WHY IT IS SHARED, NOT A WORKFLOWS SURFACE: one page hosts at least five named feature hierarchies serving four config domains - Invoice Routing (feeds the BUILT Routing Configuration page), Invoice Payment, Invoice Vendor Employee Access, Payment Authorized Approver (feeds Workflows > Authorized Approvers), Cost Object Approver (feeds the Workflows General page). Folding it into Workflows would mis-file routing and vendor access; folding it into Routing Configuration would mis-file the COA hierarchy only Workflows uses. Build it ONCE and have both point AT it. ### IT RESOLVES FOUR ALREADY-WRITTEN ENDPOINTS - dep.g1.061, dep.g2.003, dep.g5g5.050, dep.g5g5.051 - and EVERY ONE is about a SOURCE LIST ("In the Source List, select the name of the list you created in Step 1"), exactly the one field this page documents well. That fan-out across four groups is the signature of a shared page. Use the exact field name "Source List". ### HARD FORWARD REFERENCE INTO THE WORKFLOWS BUILD: "You must define the authorized approver hierarchy (Invoice > Feature Hierarchies)" - the Authorized Approver feature cannot be activated without this page. Emit that edge. ### NAVIGATION - LOAD-BEARING AND EASY TO GET WRONG. Both click paths go Administration > Invoice > Feature Hierarchies with NO middle segment. The "Invoice Processing Admin" node is UNATTESTED for this page - record as UNATTESTED, NOT ABSENT. The left-menu entry is a SIBLING of Workflows, not a child; an automation driver told to reach it via a Workflows path WILL FAIL. That is an active harm, not a filing preference. ### MENU-LABEL ALIAS NO PRIOR ARTEFACT RECORDS: hierarchies-7f68a876.md writes the label as plain "Hierarchies". Carry that alias. ### GREP HAZARD, ACUTE HERE: the nav sentences in hierarchies-7f68a876.md, professional-edition-fb3e6aa2.md and workflow-667cee21.md use U+00A0 separators with NO ">" characters at all. NEVER grep -F across a menu arrow on this page - quote around the separators. ### CRITIC CORRECTION - A GROUP CONFIGURATIONS TOPIC WAS MIS-SEEDED HERE AND IS REMOVED: overview-8b2edfd0.md was the third-largest seed and it is a GROUP CONFIGURATIONS topic ("Group Configurations in Concur Invoice allow administrators to define and manage hierarchical groups"; "The Group Configurations tool is not visible if you have the Invoice Configuration administrator (Restricted) role"). Group Configurations is BUILT (Group 1, 22 fields). Seeding it on the thinnest page in the group is the exact setup for re-homing built G1 fields, and it inflates measured richness so the thinness disguises itself. DO NOT READ IT AS A FIELD SOURCE FOR THIS PAGE. ### CRITIC CORRECTION - TWO SEEDS ADDED: tools-guides/step-1-create-the-source-list-in-list-management-cc91aa73.md, which is where the Source List (the one field this page documents well) is actually CREATED and which the roster seeded step-2 of without step-1; and tools-guides/the-import-template-fields-and-descriptions-8b51ca3d.md (47 rows) as a DEDUPE-FLAGGED REFERENCE ONLY for Level semantics - the graph already references it 4 times and Group 5 already owns its Level value set (vset.g5g5...level-1-code-level-10-code), so DO NOT re-emit that value set. ### CRITIC CORRECTION - TWO navPathEvidence ENTRIES WERE MISFILED. (a) The hierarchies-7f68a876.md quote "Hierarchies (explained below)." verifies but does NOT carry the path - the path sits in the preceding NBSP-separated clause. Use a quote spanning the separators, or demote it to an aliases citation. (b) tools-guides/overview-of-steps-37e3c289.md "Step 2: Feature Hierarchies" is NOT nav evidence but is STRONGER than the roster credited: its framing sentence is "The administrator uses the following tools in the order presented" and the list runs Step 1: List Management, Step 2: Feature Hierarchies, Step 3: Group Configurations, Step 4: Forms and Fields, Step 5: Vendor Employee Access Import, Step 6: User Administration - naming Feature Hierarchies as a peer TOOL of three already-built pages. That is first-class page-hood evidence; put it in identityNotes, not navPathEvidence. ### THE ONE LONG CATALOG IS A WORKED EXAMPLE, NOT A ROSTER: understand-the-hierarchy-b65d7089.md (69 rows) is example approver-hierarchy DATA. Say in notes that you skipped it as illustrative and why. Every field-bearing file on this page returns 0 indented table rows and 0 raw tables. ### NO NEW EXPERIENCE TWIN EXISTS. Do not claim uiVariant "new" or "both".',
  },
]
...
// ---------------------------------------------------------------------------
phase('Map')
log('Workflows Run A - mapping 2 pages: Workflows (one page, seven tabs), Feature Hierarchies')
...
const [vsRec, depRec, stepRec, ctrRec] = await parallel([
  ...
  () => agent([
    PREAMBLE,
    '',
    '=== YOUR JOB: BUILD THE ConfigSteps FOR THE WORKFLOWS GROUP ===',
    ...
    '',
    'Step ids MUST be prefixed "grpworkflows-" (this is how the merge tool namespaces them; it is derived',
    'from the group label "Workflows", which deliberately carries no "Group N" - see the script header).',
    ...
    '{ "steps": [ { "id": "grpworkflows-...", "name": "...", "goal": "<what this achieves and what silently',
...
const [criticA, criticB] = await parallel([
  ...
  () => agent([
    ...
    '    each honestly marked CORPUS-STATED or INFERRED? Are step ids prefixed grpworkflows-?',
